...
\begin{titlepage}
  ...
  {\Large\textcolor{ocpGold}{Démonstrateur de Plateforme de Maintenance Prédictive}}\\[0.2cm]
...
\begin{itemize}[leftmargin=1.5cm]
  ...
  \item Des \textbf{modèles ML} (XGBoost, Random Forest, Isolation Forest) pour la détection de dérives
  \item Un \textbf{tableau de bord centralisé} avec KPIs consolidés
...
\begin{itemize}
  \item \textbf{Moteur d'inférence (Python/FastAPI)} : exécution des modèles \texttt{joblib} pour une analyse à faible latence.
  \item \textbf{Recherche et Validation (R)} : utilisé en back-office pour le recalibrage périodique des seuils et la validation statistique des modèles.
\end{itemize}
...
\begin{itemize}
  ...
  \item \texttt{/notifications/*} -- Alertes et notifications consolidées
...
\section{Tableau de Bord Centralisé}

L'interface principale offre une vue consolidée de l'état de la flotte :

\begin{itemize}
  \item KPIs en quasi temps réel (disponibilité, indicateur de santé, alertes actives)
  \item Graphiques de tendance (dérives par mois, répartition par sous-système)
  \item Navigation structurée vers les modules d'analyse
\end{itemize}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\textwidth]{images/screen3.png}
  \caption{Tableau de bord centralisé -- Vue globale de la flotte}
\end{figure}
...
\begin{itemize}
  ...
  \item \textbf{Streaming} : réponse fluide (endpoint \texttt{/ask/stream})
\end{itemize}
...
\begin{figure}[H]
  ...
  \caption{Page Capteurs -- Monitoring continu des paramètres}
\end{figure}
...
\section{Performance \& Santé Machine (Version Consolidée)}

Le module de santé intègre un indicateur de santé dynamique calculé par un modèle \textbf{Random Forest}, l'historique des pannes, et une estimation expérimentale de la dégradation par sous-système (Moteur, Transmission, Hydraulique).
...
MineAssist constitue un prototype opérationnel avancé de maintenance prédictive adapté aux contraintes industrielles d'OCP. Le projet démontre la faisabilité technique d'intégrer des méthodes d'IA (RAG, ML) dans un contexte de maintenance industrielle.

\subsection*{Résultats clés}
\begin{itemize}
  \item \textbf{Interface centralisée} couvrant les besoins de diagnostic et de suivi
  \item \textbf{Base documentaire indexée} : CHF442, RENR6306, RENR9347, schémas
  \item \textbf{Modèles ML} de détection de dérives entraînés sur données GMAO
  \item \textbf{Démarche Shadow Mode} validant la robustesse du pipeline sur flux rejoués
\end{itemize}

\subsection*{Perspectives de ROI}
L'analyse coût-bénéfice suggère un ROI prospectif estimé à 25\% en première année, porté par la capacité de détection précoce des dérives via l'Isolation Forest, permettant d'anticiper les ruptures critiques.
