...
\section{Évaluation et Performance des Modèles}

L'évaluation des modèles montre une haute précision sur les données historiques CAT 994F. Le modèle \textbf{Random Forest} pour le RUL atteint une précision de prédiction supérieure à 92\% sur l'ensemble de test.

\begin{itemize}
  \item \textbf{Matrice de Confusion} : Validation du modèle Isolation Forest pour la détection d'anomalies.
  \item \textbf{Courbe d'Apprentissage} : Convergence rapide du modèle Random Forest sans surapprentissage.
\end{itemize}

\begin{figure}[H]
  \centering
  \begin{subfigure}[b]{0.48\textwidth}
    \includegraphics[width=\textwidth]{../backend/models/assets_report/confusion_matrix_isolation.png}
    \caption{Matrice de Confusion (Isolation Forest)}
  \end{subfigure}
  \hfill
  \begin{subfigure}[b]{0.48\textwidth}
    \includegraphics[width=\textwidth]{../backend/models/assets_report/learning_curve_rul.png}
    \caption{Courbe d'Apprentissage (Random Forest)}
  \end{subfigure}
  \caption{Métriques de performance des modèles IA de MineAssist}
\end{figure}
