...
from notification_service import (
    Alerte, NiveauAlerte, EmailConfig, WhatsAppConfig, notifier
)
from alert_detector import analyser_batch
...
async def analyser_et_notifier(
    ...
    et envoie les notifications au chef si des alertes sont trouvées.
    ...
    if not alertes:
        return NotificationResponse(
            nb_alertes=0, nb_critiques=0, nb_attentions=0,
            email_ok=None, whatsapp_ok=None, alertes=[]
        )

    # Envoi en arrière-plan pour ne pas bloquer l'API
    def _send():
        notifier(
            alertes,
            email_config=EMAIL_CFG if email else None,
            whatsapp_config=WHATSAPP_CFG if whatsapp else None,
        )
    ...
    return NotificationResponse(
        ...
        email_ok=True,   # optimiste — vérification asynchrone
        whatsapp_ok=True,
        alertes=[{
            "label":      a.label,
            "valeur":     a.valeur,
...
async def alerte_manuelle(
    ...
    background_tasks.add_task(
        notifier, [alerte], EMAIL_CFG, WHATSAPP_CFG
    )
    return {"status": "notification envoyée", "niveau": alerte.niveau.value}
...
async def tester_notifications():
    ...
    resultats = notifier([alerte_test], EMAIL_CFG, WHATSAPP_CFG)
    return {
...
async def get_seuils():
    ...
    from alert_detector import SEUILS_994F, LABELS
    return {
        param: {
