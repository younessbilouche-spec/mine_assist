...
RE_DATE_PREL = re.compile(
    r"DATE\s+DE\s+PREL[EÈ]VEMENT\s*[:\-]?\s*(\d{2}[/\-]\d{2}[/\-]\d{4})", re.I)
RE_DATE_REC = re.compile(r"DATE\s+DE\s+RECEPTION\s*[:\-]?\s*(\d{2}[/\-]\d{2}[/\-]\d{4})", re.I)
RE_DATE_FIN = re.compile(
    r"DATE\s+DE\s+FIN\s+D.ANALYSE\s*[:\-]?\s*(\d{2}[/\-]\d{2}[/\-]\d{4})", re.I)
...
def _extract_etat(text: str) -> tuple[Optional[str], Optional[str]]:
    ...
    if m_mach:
        etat_mach = m_mach.group(1).upper()
    if m_lub:
        etat_lub = m_lub.group(1).upper()
...
SEUILS = {
    "SAE 50": {
        ...
        # REF + 0.4 (réf inconnue → alerte si > 2.0)
        "tan":           {"ref_plus": 0.4},
        "fe":            (60, 250),
        "al":            (8, 16),
        "cu":            (150, 900),
