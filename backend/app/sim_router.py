...
from app.seuils_ocp import (
    analyser_snapshot_ocp,
    covered_sensors as _ocp_covered_sensors,
    _normalize_param as _ocp_normalize,
    info as _ocp_info,
)
...
def _detect_alerts_async(payload: list[dict], engin: str, ts: datetime,
    ...
    """
    # 1) detection ----------------------------------------------------
    #    a) seuils officiels OCP (regles conditionnelles rpm + hyd_load)
    #    b) seuils legacy CAPTEUR_THRESHOLDS (uniquement pour les capteurs
    #       NON couverts par l'OCP, pour eviter le double-fire)
    snapshot = {p["parametre"]: p["val_max"] for p in payload}
    try:
        alertes_ocp = analyser_snapshot_ocp(
            snapshot, engin=engin, ts=ts, cycle_phase=cycle_phase,
        )
    except Exception as e:
        print(f"[sim_router] erreur analyser_snapshot_ocp : {e}")
        alertes_ocp = []

    try:
        alertes_legacy = analyser_batch(payload)
    except Exception as e:
        print(f"[sim_router] erreur analyser_batch : {e}")
        alertes_legacy = []

    # Filtre legacy : on garde uniquement les capteurs NON couverts par OCP
    covered = _ocp_covered_sensors()
    alertes_legacy_filtered = [
        a for a in alertes_legacy
        if _ocp_normalize(getattr(a, "parametre", "")) not in covered
    ]
    alertes_obj = list(alertes_ocp) + alertes_legacy_filtered
...
@sim_router.get("/seuils-ocp")
async def sim_seuils_ocp():
    """
    Diagnostic : renvoie les seuils OCP charges (13 mappables) + capteurs couverts.
    Utile pour verifier que seuils_OCP.json est bien deploye cote backend.
    """
    return _ocp_info()
