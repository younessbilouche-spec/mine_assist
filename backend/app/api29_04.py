...
from app.schema_locator import extract_schema_crops_for_diagnosis
...
def gmao_timeseries(
    ...
    machine: str = Query("all", description="Machine à filtrer ('994F-1', '994F-2', ou 'all')"),
    last_minutes: int = Query(0, description="Si > 0, ne renvoyer que les N dernières minutes"),
    resample: str = Query("auto", description="Période de resampling pandas ('1min', '5min', '1h', 'auto')"),
    max_points: int = Query(500, description="Nombre maximum de points à renvoyer (downsample si dépassé)"),
):
    """
    Retourne une série temporelle propre (min/moy/max) pour un paramètre donné.
    Conçu pour alimenter les courbes live du MonitoringDashboard frontend.
    """
    try:
        df, _ = load_gmao_capteurs()
        ...
        df_p = df[mask].copy()
        if df_p.empty:
            raise HTTPException(404, f"Paramètre '{parametre}' non trouvé dans les fichiers capteurs")
        ...
        if machine and machine != "all":
            ...
            if df_p.empty:
                raise HTTPException(
                    404, f"Aucune mesure pour le paramètre '{parametre}' sur la machine '{machine}'"
                )
        ...
        if last_minutes and last_minutes > 0:
            ...
            if df_p.empty:
                raise HTTPException(
                    404, f"Aucune mesure dans les {last_minutes} dernières minutes"
                )

        # ── 4. Resampling intelligent ────────────────────────────────────────
        # 'auto' : choisit la fenêtre selon l'étendue temporelle pour ~max_points points
        if resample == "auto":
            span_min = (df_p["horodatage"].max() - df_p["horodatage"].min()).total_seconds() / 60
            target_min = max(1, span_min / max_points)  # minutes par point
        ...
        # Si on a malgré tout dépassé max_points, downsample uniformément
...
# ─────────────────────────────────────────────────────────────────────────────
# BONUS : endpoint /gmao/timeseries/multi pour récupérer plusieurs paramètres
# d'un coup (utile pour le radar live et le dashboard de comparaison)
# ─────────────────────────────────────────────────────────────────────────────
...
def gmao_timeseries_multi(
    ...
    """
    Retourne plusieurs séries temporelles en une seule requête.
    Exemple : /gmao/timeseries/multi?parametres=Température,Pression&machine=994F-1
    """
...
def diagnose(req: DiagnoseRequest):
    try:
        ...
        import re
        code_keywords = re.sub(r'\b(MID|CID|FMI|EID)\s*\d+\b', '', code, flags=re.IGNORECASE).strip()
        CID_MEANINGS = {
            "168": "tension électrique système batterie alternateur voltage",
            "110": "température liquide refroidissement moteur",
            "190": "régime vitesse moteur speed",
            "273": "pression turbocompresseur turbo outlet",
            "277": "étalonnage calage timing capteur",
            "529": "pression huile moteur oil pressure",
            "542": "pression huile moteur non filtrée",
            "543": "tension électrique système voltage",
            "544": "température sortie convertisseur",
            "545": "régime sortie convertisseur",
            "670": "pédale convertisseur couple position",
            "767": "pression pompe équipement hydraulique",
            "800": "module VIMS communication",
            "296": "transmission communication data link",
            "603": "pression embrayage impeller convertisseur",
        }

        import re
        cid_match = re.search(r'CID\s*(\d+)', code, re.IGNORECASE)
        eid_match = re.search(r'EID\s*(\d+)', code, re.IGNORECASE)
        code_enrichment = ""
        if cid_match:
            code_enrichment = CID_MEANINGS.get(cid_match.group(1), "")
        elif eid_match:
            eid_map = {
                "95": "filtre carburant restriction fuel filter",
                "17": "température liquide refroidissement surchauffe",
                "98": "prélubrification moteur prelube",
                "72": "niveau huile moteur insuffisant",
                "73": "pression filtre huile différentiel",
                "334": "pression lubrification automatique faible",
            }
            code_enrichment = eid_map.get(eid_match.group(1), "")

        query = f"{code} {code_enrichment} {symptoms_text}".strip()
        context, sources = rag.build_context(
            query=query,
            top_k=TOP_K,
            max_chars=MAX_CHARS_CONTEXT,
        ...
        user_prompt = f"""## Cas de diagnostic — CAT 994F
        ...
        ## INSTRUCTION
        Si le contexte documentaire contient des valeurs réelles de capteurs (moyennes, min, max, nombre de dépassements, tendances mensuelles), 
        tu DOIS les citer explicitement dans ton diagnostic avec leurs unités.
        Ne dis pas "vérifier les valeurs" si tu as déjà les valeurs — donne-les directement.

{ctx_block}{sources_block}"""

        answer = call_llm(system, user_prompt)
        pdf_images = extract_images_for_sources(sources, query=query, max_images=2)

        # ── Localisation dans les schémas hydrauliques / électriques ────────
        schema_dir = str(BASE_DIR / "data" / "schemas")
        schema_crops = extract_schema_crops_for_diagnosis(
