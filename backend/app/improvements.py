...
2. **Source du savoir** :
    ...
    b) Si CONTEXT est vide, peu pertinent, ou s'il lui MANQUE une information précise
       (ex: une capacité, un couple de serrage, une étape), cherche impérativement
       dans la **BASE DE CONNAISSANCES EMBARQUÉE CAT 994F** ci-dessous.
    c) Ne dis JAMAIS que l'information n'est pas disponible si elle figure dans
       la BASE EMBARQUÉE.
    d) JAMAIS inventer un n° de pièce CAT exact. Si tu ne connais pas la valeur exacte
       ni dans le CONTEXT ni dans la BASE EMBARQUÉE, écris-la « (à valider sur SIS / VIMS) ».
...
**EXCEPTION** : Si l'utilisateur dit juste bonjour ou pose une question de pure politesse, réponds naturellement et très brièvement SANS utiliser les 5 sections."""
...
Périodicité : PM250 (250 h). Capacité ~265 L huile CAT DEO 15W-40 (à vérifier sur SIS selon climat). Couple bouchon : voir SIS.
Procédure : 1) Moteur tiède, EPI. 2) LOTO. 3) Bac de récup grande capacité. 4) Dévisser bouchon vidange carter. 5) Récupérer toute l'huile (~15 min). 6) Visser bouchon (couple SIS). 7) Remplir avec capacité préconisée. 8) Démarrer, vérifier témoin pression, couper, contrôler niveau. 9) Fiche entretien VIMS + cuve d'usage.

[VÉRIFICATION NIVEAU HUILE MOTEUR]
Procédure : 1) Moteur à l'arrêt sur sol plat, attendre 5-10 min après arrêt. 2) Accéder à la jauge (dipstick) côté droit moteur. 3) Extraire la jauge, l'essuyer avec un chiffon propre. 4) Réinsérer complètement la jauge, puis l'extraire à nouveau. 5) Vérifier que le niveau se situe entre les marques "ADD" et "FULL". 6) Si niveau bas, ajouter de l'huile CAT DEO 15W-40 par l'orifice de remplissage. 7) Ne jamais dépasser le repère "FULL".

[CODE ÉVÉNEMENT E102 — BASSE PRESSION D'HUILE MOTEUR]
Gravité : Élevée (Risque de casse moteur immédiat).
Causes possibles : Niveau d'huile bas, pompe à huile défaillante, capteur défectueux, colmatage filtre.
Procédure : 1) Arrêt moteur immédiat. 2) Vérification manuelle du niveau d'huile. 3) Inspection visuelle de fuites majeures. 4) Si niveau OK, tester le capteur de pression d'huile avec un manomètre externe (valider sur SIS). 5) Remplacer filtres si colmatés.

[ENTRETIEN PM1000 — 1000 HEURES]
Périodicité : Toutes les 1000 heures de fonctionnement.
Opérations clés : 1) Vidange huile moteur + filtres. 2) Remplacement filtres à carburant (primaire et secondaire). 3) Remplacement filtres hydrauliques (transmission, direction, frein). 4) Inspection des courroies et de la tension. 5) Nettoyage des reniflards de carter. 6) Prélèvement d'huile S·O·S (moteur, hydraulique, transmissions). 7) Inspection des points d'articulation et graissage complet.
Outils : Kit PM1000 CAT, valise ET pour diagnostic électronique.
...
**EXCEPTION**: If the user just says hello or asks a purely non-technical question, answer naturally and briefly WITHOUT using the 5 sections.""" + """
...
**استثناء**: إذا قال المستخدم مجرد تحية أو سؤال غير تقني، أجب بشكل طبيعي وقصير بدون استخدام الأقسام الخمسة.""" + """
...
def register_improvements(app: FastAPI):
        ...
        if not has_ctx:
            ...
            note = {
                "fr": "\n\n(IMPORTANT : la recherche RAG n'a pas trouvé de chunk pertinent. "
                      "Si la question de l'utilisateur n'est PAS technique (ex: un bonjour), réponds normalement et brièvement. "
                      "SINON, ne dis JAMAIS « documentation non consultée ». À la place, fournis une "
                ...
                "en": "\n\n(IMPORTANT: RAG search returned no relevant chunks. "
                      "If the user is just saying hello, answer naturally. "
                      "OTHERWISE, NEVER say 'documentation not consulted'. Instead, provide a COMPLETE STRUCTURED "
    ...
    def ask_stream(req: AskV2Request, request: Request):
        ...
        if not has_ctx:
            ...
            note_stream = {
                "fr": "\n\n(IMPORTANT : pas de chunk RAG trouvé. S'il s'agit d'un simple bonjour, réponds brièvement SANS structure. "
                      "SINON, NE DIS PAS « documentation non consultée ». Utilise la BASE CAT 994F EMBARQUÉE et donne une "
                      "procédure complète en 5 sections.)",
                "en": "\n\n(IMPORTANT: no RAG chunk found. If it's a simple greeting, answer briefly WITHOUT structure. "
                      "OTHERWISE, DO NOT say 'documentation not consulted'. Use the EMBEDDED CAT 994F base and give a complete "
                      "5-section procedure.)",
                "ar": "\n\n(مهم: لم يُعثر على مقاطع RAG. إذا كانت مجرد تحية، أجب باختصار بدون هيكل. "
                      "وإلا، لا تقل «لم تُستشار الوثائق». استخدم قاعدة CAT 994F المضمّنة وقدم إجراءً كاملاً في 5 أقسام.)",
            }
