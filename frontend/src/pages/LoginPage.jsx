...
function OcpLogo({ compact = false }) {
  ...
  if (failed) {
    return <div style={{ background: compact ? C.green : "white", color: compact ? "white" : C.green, borderRadius: 12, padding: compact ? "10px 12px" : "12px 18px", fontWeight: 900, letterSpacing: 3 }}>OCP</div>
  }
  return (
    <div style={{
      width: compact ? 58 : 92,
      ...
      border: compact ? "1px solid rgba(255,255,255,.45)" : `1px solid ${C.border}`,
      display: "flex",
      alignItems: "center",
      justifyContent: "center",
      boxShadow: compact ? "0 12px 28px rgba(0,0,0,.18)" : "0 14px 35px rgba(0,132,61,.16)",
      overflow: "hidden",
    }}>
      <img src={ocpLogo} alt="OCP" onError={() => setFailed(true)} style={{ width: compact ? 48 : 78, height: compact ? 48 : 78, objectFit: "contain" }} />
    </div>
  )
}
        ...
        <main style={{ display: "flex", alignItems: "center", justifyContent: "center", padding: 42 }}>
          <form onSubmit={submit} style={{ width: "100%", maxWidth: 460, background: "rgba(255,253,248,.92)", border: `1px solid ${C.border}`, borderTop: `5px solid ${C.green}`, borderRadius: 22, padding: "38px 40px", boxShadow: "0 26px 80px rgba(77,60,33,.18)", backdropFilter: "blur(18px)" }}>
            <div style={{ textAlign: "center", marginBottom: 30 }}>
              <div style={{ display: "flex", justifyContent: "center", marginBottom: 16 }}><OcpLogo /></div>
