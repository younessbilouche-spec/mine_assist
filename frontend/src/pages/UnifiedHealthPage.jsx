...
import OcpHealthPage from "./OcpHealthPage"
import MLHealthHistoryPage from "./MLHealthHistoryPage"
...
export default function UnifiedHealthPage({ apiFetch, onNavigate }) {
  ...
  return (
    <div style={{ position: "relative", zIndex: 1 }}>
      <div style={{ background: "rgba(245,240,232,0.98)", borderBottom: `1px solid ${C.border}`, padding: "10px 32px 0" }}>
        <div style={{ maxWidth: 1240, margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, fontFamily: "'Rajdhani', sans-serif" }}>
            ...
            <div>
              <div style={{ fontSize: 11, fontWeight: 700, color: C.textMuted, letterSpacing: 4, textTransform: "uppercase" }}>PERFORMANCE & SANTÉ</div>
      ...
      <div style={{ background: "rgba(255,253,248,0.4)" }}>
        {mode === "current" ? (
          <OcpHealthPage apiFetch={apiFetch} onNavigate={onNavigate} />
        ) : (
          <MLHealthHistoryPage />
        )}
      </div>
